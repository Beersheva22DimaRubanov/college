...
public class CollegeServiceImpl implements CollegeService {
	...
	private <T> Long getStudentUniqueId() {
		return getId(studentRepo);
	}

	private <T> Long getLecturerUniqueId() {
	...
	public SubjectDto addSubject(SubjectDto subjectDto) {
		if (subjectRepo.existsById(subjectDto.getId())) {
			throw new IllegalStateException("Subject with given id exsists " + subjectDto.getId());
		}
		Lecturer lecturer = null;
		Long lecturerId = subjectDto.getLecturerId();
		if (lecturerId != null) {
			lecturer = lecturerRepo.findById(lecturerId).orElseThrow(
					() -> new NotFoundException(String.format("Lecturer with id %d doesn't exsist", lecturerId)));
		}
		Subject subject = Subject.of(subjectDto);
		subject.setLecturer(lecturer);
		SubjectDto res = subjectRepo.save(subject).build();
	...
	public MarkDto addMark(MarkDto mark) {
		...
		Student student = studentRepo.findById(studentId).orElseThrow(
				() -> new NotFoundException(String.format("Student with id %d is not exist in DB", studentId)));
	...
	@Override
	@Transactional(readOnly = false)
	public SubjectDto updateLecturer(String subjectId, Long lectureId) {
		Subject subject = subjectRepo.findById(subjectId).orElseThrow(
				() -> new NotFoundException(String.format("Subject with id %s doesn't exist in DB", subjectId)));
		Lecturer lecturer = null;
		if (lectureId != null) {
			lecturer = lecturerRepo.findById(lectureId).orElseThrow(
					() -> new NotFoundException(String.format("Subject with id %s doesn't exist in DB", lectureId)));
		}
		subject.setLecturer(lecturer);
		return subject.build();
	}
